...
import { TReleaseInsert, TReleaseUpdate, TTrackInsertForm, TTrackUpdateForm } from 'shared/schema/release.schema';
...
export async function updateRelease(releaseId: string, releaseData: TrueOmit<TReleaseUpdate, 'preview' | 'tracks'> & {
		preview: string;
	} & {
		tracks: (TrueOmit<
			TTrackUpdateForm,
	...
	const NewRelize = await functional.api.v1.releases.updateRelease(
		connection,
		releaseId,
		{
			release:{
